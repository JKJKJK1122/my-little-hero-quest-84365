...
const getSampleScenarios = (theme: string, difficulty: 'beginner' | 'intermediate' | 'advanced') => {
  // GameScenarios.ts에서 실제 데이터 가져오기
  const themeData = SCENARIOS[theme as keyof typeof SCENARIOS];
  
  if (!themeData) return [];
  
  const scenariosArray: any[] = [];
  
  // 각 시나리오 카테고리를 순회
  for (const categoryKey of Object.keys(themeData)) {
    const category = themeData[categoryKey as keyof typeof themeData];
    
    if (category && typeof category === 'object' && difficulty in category) {
      const difficultyData = category[difficulty as keyof typeof category];
      
      if (difficultyData && typeof difficultyData === 'object' && 'title' in difficultyData) {
        scenariosArray.push({
          title: difficultyData.title,
          situation: difficultyData.situation,
          options: difficultyData.options.map((opt: any) => opt.text),
          correctOption: difficultyData.options.findIndex((opt: any) => opt.is_correct)
        });
      }
    }
  }
